...
; Commands
(define custom-tactics? #t)
(define (enable-custom-tactics) (set! custom-tactics? #t))
(define (disable-custom-tactics) (set! custom-tactics? #f))

(define (set-option opt val) (printf-smt "(set-option ~a ~a)\n" opt val))

(define (set-logic l)    (printf-smt "(set-logic ~a)\n" l))
(define (check-sat)      (if custom-tactics?
                             (printf-smt "(check-sat-using (par-or (then simplify bit-blast smt) (then simplify reduce-bv-size smt) (then simplify max-bv-sharing smt) (then simplify reduce-bv-size max-bv-sharing smt) (then simplify reduce-bv-size max-bv-sharing bit-blast smt) (then simplify elim-small-bv reduce-bv-size max-bv-sharing bv_bound_chk bit-blast smt) smt))\n")
