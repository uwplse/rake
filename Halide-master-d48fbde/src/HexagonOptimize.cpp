...
private:
    ...
    Expr visit(const Cast *op) override {
        ...
        static const vector<Pattern> casts = {
            ...
            {"halide.hexagon.trunc_satub_rnd.vuh", u8_sat(rounding_shift_right(wild_u16x, u16(8))), Pattern::DeinterleaveOp0},
            ...
            {"halide.hexagon.trunc_satuh_rnd.vuw", u16_sat(rounding_shift_right(wild_u32x, u32(16))), Pattern::DeinterleaveOp0},
...
private:
    ...
    public:
        ...
        std::string visit(const Call *op) {
            ...
            } else if (op->name == std::string("widening_mul")) {
                std::string rkt_args = "";

                indent.push(indent.top() + 1);
                for (unsigned int i = 0; i < op->args.size(); i++)
                    rkt_args += "\n" + dispatch(Cast::make(op->type, op->args[i]));
                indent.pop();
    ...
    public:
        ...
        InferSymbolics(std::map<std::string, Expr> lvs, Scope<Interval> &bnds, FuncValueBounds fvb)
...
Stmt optimize_hexagon_instructions_legacy(Stmt s, const Target &t) {
