...
public:
    ...
    Expr mutate(const Expr &expr) override {
        ...
        for (auto const &e : mutated_exprs) {
            if (e == expr.get()) {
                found = true;
                break;
            }
    ...
    OptimizePatterns(const Target &t, std::set<const BaseExprNode *> &ms)
        : target(t), mutated_exprs(ms) {
    }
};
...
class EliminateInterleaves : public IRMutator {
    ...
    // Expressions mutated by Rake, don't touch these
    std::set<const BaseExprNode *> &mutated_exprs;
...
public:
    ...
    EliminateInterleaves(int native_vector_bytes, std::set<const BaseExprNode *> &ms)
        : native_vector_bits(native_vector_bytes * 8), alignment_analyzer(native_vector_bytes), mutated_exprs(ms) {
    }
};
...
Stmt optimize_hexagon_instructions_legacy(Stmt s, const Target &t, std::set<const BaseExprNode *> &mutated_exprs) {
