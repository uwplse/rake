  ...
  (decode
   (parameterize ([current-readtable (make-readtable #f #\# #\a #f)]) ; read BV literals as symbols
     (match (server-read server (read))
       ...
       [(== 'unknown) 'unknown]
       [other
        (define str (format "~a" other))
        ;; Check if the solver was cancelled by the timeout
        (cond
          [(regexp-match #rx"line (.+?) column (.+?): canceled" str) 'unknown]
          [else (error 'read-solution "unrecognized solver output: ~a" other)])]))
   env))
