...
(define t1025.s (var-lookup 't1025.s (sca-add  t1588  t1587)))
...
(define halide-expr
 (vec-add
  (vector_reduce 'add 4
   (int32x128
    (vec-mul
     (uint16x128
      (vec-broadcast 32
       (load input (ramp (sca-add (sca-mul convolved.s1.r19$z.r124 4) (sca-add (sca-mul input.stride.1 t1025.s) t1013)) 1 4) (aligned 4 0))))
     (uint16x128
  ...
  (load convolved (ramp 384 1 32) (aligned 0 384))))

(define spec (synthesis-spec 'halide-ir halide-expr axioms))
(define hvx-expr (synthesize-hvx spec 'greedy 'enumerative 'enumerative))

(llvm-codegen hvx-expr "sexp_86.out")