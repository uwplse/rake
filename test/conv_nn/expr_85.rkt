...
(define-symbolic t1590 integer?)
(define-symbolic t1589 integer?)
...
(define t1024.s (var-lookup 't1024.s (sca-add  t1590  t1589)))
...
(define halide-expr
 (vec-add
  (vector_reduce 'add 4
   (int32x128
    (vec-mul
     (uint16x128
      (vec-broadcast 32
       (load input (ramp (sca-add (sca-mul convolved.s1.r19$z.r124 4) (sca-add (sca-mul input.stride.1 t1024.s) t1013)) 1 4) (aligned 4 0))))
     (uint16x128
  ...
  (load convolved (ramp 352 1 32) (aligned 0 352))))

(define spec (synthesis-spec 'halide-ir halide-expr axioms))
(define hvx-expr (synthesize-hvx spec 'greedy 'enumerative 'enumerative))

(llvm-codegen hvx-expr "sexp_85.out")