...
(require rake)
(init-logging "expr_114.runtimes")
...
(define-symbolic t1769 integer?)
(define-symbolic t1770 integer?)
...
(define t1117.s (var-lookup 't1117.s (sca-add  t1770  t1769)))
...
(define halide-expr
 (vec-add
  (vector_reduce 'add 4
   (int32x128
    (vec-mul
     (uint16x128
      (vec-broadcast 32
       (load input (ramp (sca-add (sca-mul convolved.s1.r19$z.r124 4) (sca-add (sca-mul input.stride.1 t1117.s) t1108)) 1 4) (aligned 4 0))))
     (uint16x128
  ...
  (load convolved (ramp 192 1 32) (aligned 0 192))))

(define spec (synthesis-spec 'halide-ir halide-expr axioms))
(define hvx-expr (synthesize-hvx spec 'greedy 'enumerative 'enumerative))

(llvm-codegen hvx-expr "sexp_114.out")