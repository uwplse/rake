...
(require rake)
(init-logging "expr_128.runtimes")
...
(define t1157 (var-lookup 't1157 (sca-add  t1866  t1865)))
  ...
  (vector_reduce 'add 4
   (int32x128
    (vec-mul
     ...
     (uint16x128
      (load filter (ramp (sca-add (sca-mul (sca-add (sca-mul convolved.s1.r19$z.r124 32) (sca-mul filter.stride.3 output.s0.c.co)) 4) t1157) 1 128) (aligned 128 0))))))
  (load convolved (ramp 0 1 32) (aligned 0 0))))

(define spec (synthesis-spec 'halide-ir halide-expr axioms))
(define hvx-expr (synthesize-hvx spec 'greedy 'enumerative 'enumerative))

(llvm-codegen hvx-expr "sexp_128.out")