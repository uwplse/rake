...
(define t929.s (var-lookup 't929.s (sca-add  t1378  t1377)))
...
(define halide-expr
 (vec-add
  (vector_reduce 'add 4
   (int32x128
    (vec-mul
     (uint16x128
      (vec-broadcast 32
       (load input (ramp (sca-add (sca-mul convolved.s1.r19$z.r124 4) (sca-add (sca-mul input.stride.1 t929.s) t915)) 1 4) (aligned 4 0))))
     (uint16x128
  ...
  (load convolved (ramp 576 1 32) (aligned 0 576))))

(define spec (synthesis-spec 'halide-ir halide-expr axioms))
(define hvx-expr (synthesize-hvx spec 'greedy 'enumerative 'enumerative))

(llvm-codegen hvx-expr "sexp_44.out")