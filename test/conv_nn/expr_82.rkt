...
(define-symbolic t1591 integer?)
(define-symbolic t1592 integer?)
...
(define t1023.s (var-lookup 't1023.s (sca-add  t1592  t1591)))
...
(define halide-expr
 (vec-add
  (vector_reduce 'add 4
   (int32x128
    (vec-mul
     (uint16x128
      (vec-broadcast 32
       (load input (ramp (sca-add (sca-mul convolved.s1.r19$z.r124 4) (sca-add (sca-mul input.stride.1 t1023.s) t1013)) 1 4) (aligned 4 0))))
     (uint16x128
  ...
  (load convolved (ramp 256 1 32) (aligned 0 256))))

(define spec (synthesis-spec 'halide-ir halide-expr axioms))
(define hvx-expr (synthesize-hvx spec 'greedy 'enumerative 'enumerative))

(llvm-codegen hvx-expr "sexp_82.out")