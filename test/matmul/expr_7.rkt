...
(define halide-expr
 (let ([t180  (int32x1  (sca-max  (sca-min  mat_a_offset  (int16_t (bv 0 16)))  (int16_t (bv -255 16))))])
  (let ([t181   (int32x1   (sca-max   (sca-min   mat_b_offset   (int16_t (bv 0 16)))   (int16_t (bv -255 16))))])
   (let ([t182    (vec-mul
    (int64x128
     (vec-add
      (vec-add
      ...
      (x128 (sca-add (sca-mul (sca-mul t180 t181) mat_a.extent.0) (sca-mul t181 (int32x1 (load-sca row_sums_a (sca-add (sca-mul (sca-sub output.s0.y.yo (sca-min (sca-div output.extent.1 4) 8)) 4) 33))))))))
    (int64x128
