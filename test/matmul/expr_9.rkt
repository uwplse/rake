...
(define halide-expr
 (let ([t194  (int32x1  (sca-max  (sca-min  mat_a_offset  (int16_t (bv 0 16)))  (int16_t (bv -255 16))))])
  (let ([t195   (int32x1   (sca-max   (sca-min   mat_b_offset   (int16_t (bv 0 16)))   (int16_t (bv -255 16))))])
   (let ([t196    (vec-mul
    (int64x128
     (vec-add
      (vec-add
      ...
      (x128 (sca-add (sca-mul (sca-mul t194 t195) mat_a.extent.0) (sca-mul t195 (int32x1 (load-sca row_sums_a (sca-add (sca-mul (sca-sub output.s0.y.yo (sca-min (sca-div output.extent.1 4) 8)) 4) 35))))))))
    (int64x128
