...
int main(int argc, char **argv) {
  ...
  char *filename = (char *) malloc(100 * sizeof(char));
    ...
    halide_dimension_t xo_dim{2, width-2, 1};
    halide_dimension_t yo_dim{2, height-2, width};
    ...
    cycles_hvx64 = benchmark([&]() {
        int error = gaussian5x5_hvx64(input1_buf, output_buf);
        if (error != 0) {
    ...
    printf("AppReported (HVX64B-mode): Image %dx%d - gaussian5x5(64B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_hvx64/width/height);
    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "gaussian5x5_hvx64_");
    ...
    printf("AppReported (HVX128B-mode): Image %dx%d - gaussian5x5(128B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_hvx128/width/height);
    strcpy(filename, "gaussian5x5_hvx128_");
    strcat(filename, argv[4]);
