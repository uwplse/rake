...
int main(int argc, char* argv[])
{
    ...
    boxfilter11x11( input, stride, width, height, output );

    total_cycles = READ_PCYCLES() - start_time;
    DUMP_PMU();

#if defined(__hexagon__)
    printf("AppReported (HVX%db-mode): Image %dx%d - boxfilter11x11: %0.4f cycles/pixel\n", VLEN, (int)width, (int)height, (float)total_cycles/width/height);
#endif
    /* -----------------------------------------------------*/
    ...
    for(i = 0; i < height; i++)
    {
        if(write(fp, &output[i*stride], sizeof(unsigned char)*width)!=width)
        {
