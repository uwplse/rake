    ...
    (match ir-expr
      [(packhi sub-expr signed?)
       (define-values (successful? hvx-sub-expr)
         (synthesize-hvx-expr halide-expr halide-expr-axioms sub-expr ir-expr-sol ir-annotations lowering-algo swizzling-algo))
      ...
      [(convolve-acc acc sub-expr kernel saturateFunc outputType)
       (define-values (successful1? hvx-acc)
         (synthesize-hvx-expr halide-expr halide-expr-axioms acc ir-expr-sol ir-annotations lowering-algo swizzling-algo))
...
;         (synthesize-equiv-hvx spec sub-expr hvx-sub-expr discarded-sols))]
      ...
      [(abs-diff sub-expr1 sub-expr2)
       (define-values (successful1? hvx-sub-expr1)
         (synthesize-hvx-expr halide-expr halide-expr-axioms sub-expr1 ir-expr-sol ir-annotations lowering-algo swizzling-algo))
       (define-values (successful2? hvx-sub-expr2)
         (synthesize-hvx-expr halide-expr halide-expr-axioms sub-expr2 ir-expr-sol ir-annotations lowering-algo swizzling-algo))
      ...
      [(load-data opts) (values #t (gather* opts))]
     ...
     (cond
       [(eq? hal-elem-cnt (* 2 hvx-elem-cnt))
        (define-values (successful2? hvx-expr2)
          (synthesize-hvx-swizzles halide-expr hvx-expr-sketch (symbolics halide-expr) halide-expr-axioms swizzling-algo #t))
        (cond
