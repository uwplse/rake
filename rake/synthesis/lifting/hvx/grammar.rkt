  ...
  (define candidates
    (destruct lifted-sub-expr
      ...
      [(vs-mpy-add sub-expr weight-matrix output-type saturate?)
       (define mul-scalars (append (extract-mul-scalars-halide halide-expr) (list (int8_t (bv 1 8)))))
       ...
       (cond
         [(halide-cast-op? halide-expr)
          ...
          (define wider-type (if (>= (type-bw in-type) (type-bw narrower-type)) in-type narrower-type))
      ...
      [(add-const sub-expr const-val output-type saturate?)
       ...
       (define narrower-type (if (<= (type-bw e-type) (type-bw output-type)) e-type output-type))
       (define wider-type (if (>= (type-bw in-type) (type-bw narrower-type)) in-type narrower-type))
       (define add-scalars (extract-add-scalars-halide halide-expr))
       (list
      ...
      [(vs-shift-right sub-expr shift round? saturate? arithmetic? output-type)
       (define shr-scalars (extract-shr-scalars-halide halide-expr))
       (define e-type (halide-elem-type halide-expr))
       (define in-type (hvx-ir-elem-type sub-expr))
       (define narrower-type (if (<= (type-bw e-type) (type-bw output-type)) e-type output-type))
       (define wider-type (if (> (quotient (type-bw in-type) 2) (type-bw narrower-type)) output-type narrower-type))
       (list
  ...
  (define candidates
    (destruct lifted-sub-expr
      ...
      [(vs-mpy-add sub-expr weight-matrix output-type saturate?)
       (flatten
        (list
         (mk-average-instr sub-expr (choose* #t #f) (halide-elem-type halide-expr) (extract-shr-scalars-halide halide-expr))
      ...
      [(add-const sub-expr const-val output-type saturate?)
       ...
       (flatten
        (list
         ...
         (mk-shr-instr sub-expr shr-scalars (choose* #t #f) (choose* #t #f) (signed-type? output-type) output-type)))]
...
(define (extend-grammar lifted-sub-exprs halide-expr)
  (destruct halide-expr
    ...
    [(vec-add v1 v2)
     (define add-scalars (extract-add-scalars-halide halide-expr))
     (define add-consts (filter (lambda (scalar) (not (symbolic? scalar))) add-scalars))
     (define mul-scalars (extract-mul-scalars-halide halide-expr))
     (flatten
      (list
    ...
    [(vec-shl v1 v2)
     (define mul-scalars (extract-mul-scalars-halide halide-expr))
     (define shl-scalars (extract-mul-scalars-halide halide-expr))
     (flatten
      (list
       ;; We can extend using either vector-scalar multiply-add
       (mk-vs-mpy-add-instr (first lifted-sub-exprs) mul-scalars (halide-elem-type halide-expr) #f)
    ...
    [(vec-div v1 v2)
     (define shr-scalars (extract-shr-scalars-halide halide-expr))
     (define div-scalars (extract-div-scalars-halide halide-expr))
     (define output-type (halide-elem-type halide-expr))
     (flatten
      (list
       ;; We can extend using either a shift-right instruction
       (mk-shr-instr (first lifted-sub-exprs) shr-scalars #f #f (signed-type? output-type) output-type)
    ...
    [(vec-mod v1 v2)
     (define mod-scalars (extract-mod-scalars-halide halide-expr))
     (flatten
      (list
