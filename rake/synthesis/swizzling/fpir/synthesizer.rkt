...
(provide
  (rename-out
    [synthesize-translation fpir:swizzling:synthesize-translation]
))
...
(define (synthesize-translation templates halide-expr fpir-sub-exprs value-bounds translation-history output-layout)
  (cond
    ...
    [(hash-has-key? synthesis-db (list (first templates) halide-expr output-layout))
     ...
     (synthesize-translation (rest templates) halide-expr fpir-sub-exprs value-bounds translation-history output-layout)]
    [else
     (define template (first templates))
     (println "calling run-synthesizer")
     (define sol (run-synthesizer (car template) halide-expr fpir-sub-exprs value-bounds translation-history output-layout))
...
(define (run-synthesizer template halide-expr fpir-sub-exprs value-bounds translation-history output-layout)
  ;(println halide-expr)
  ;(pretty-print template)

  ;(define-values (optimized-halide-expr optimized-template inferred-axioms)
    ;(optimize-query halide-expr template fpir-sub-exprs value-bounds translation-history))
  ...
  (define lanes-to-verify (verification-lanes (fpir:type (fpir:interpret template))))
...
(define (synthesize-incremental halide-expr template output-layout sym-consts lanes-to-verify discarded-sols)
  (cond
    ...
    [else
     ...
     (display (format "(fpir:swizzle) Verifying lane: ~a\n" curr-lane))
     (define ihalide (halide:interpret halide-expr))
     (define irake (fpir:interpret template))
     ...
     (define sol (synthesize #:forall (symbolics halide-expr)
                             #:guarantee (begin
                                           ...
                                           (lane-eq? (halide:interpret halide-expr) (fpir:interpret template) output-layout curr-lane))))
...
(define (lane-eq? oe se output-layout lane)
  ; (define offset (quotient (fpir:num-elems se) 2))
  (cond
    [(eq? output-layout 'in-order)
      (set-curr-cn! lane)
      (assert (eq? (oe lane) (fpir:get-element se lane)))]
    [else
     (error (format "fpir:swizzling:lane-eq? only supports output-layouts: ['in-order], recieved ~a" output-layout))]))
