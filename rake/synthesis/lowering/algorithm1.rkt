...
(require
  ...
  rake/synthesis/lowering/fpir/grammar
...
(provide synthesize-hvx-template synthesize-arm-template synthesize-x86-template synthesize-fpir-template)
...
(define (synthesize-fpir-template halide-expr ir-expr fpir-sub-exprs output-layout value-bounds translation-history lowering-algo cost-ub)
  (cond
    [(eq? lowering-algo 'enumerative)
      (display "Lowering IR to FPIR...\n")
      (display "====================\n\n")
      (display (format "IR Operation: \n\n~a\n\n" (pretty-format ir-expr)))
      ; (display (format "sub-exprs: \n\n~a\n\n" (pretty-format fpir-sub-exprs)))

      (define start-time (current-seconds))
      (define candidates (get-fpir-grammar halide-expr ir-expr fpir-sub-exprs output-layout cost-ub))
      (define runtime (- (current-seconds) start-time))

      (display (format "Template candidates: ~a\n" (length candidates)))
      (display (format "Template enumeration time: ~a seconds\n\n" runtime))
      
      (define-values (successful? fpir-template)
        (fpir:synthesize-translation candidates halide-expr fpir-sub-exprs value-bounds translation-history))
      
      (display "Finished synthesizing translation\n")

      (cond
        [successful?
          (display "\nSuccessfully found an equivalent fpir template.\n\n")
          (display (format "~a\n" (pretty-format (car fpir-template))))
          (display "\nSynthesis time: 0 seconds\n\n")
          (values #t (car fpir-template) (cdr fpir-template))]
        [else
          (display "Failed to synthesize template.\n\n")
          (values #f (void) 0)])]
    [else (error (format "Unrecognized lowering algorithm specified: '~a. Supported algorithms: ['enumerative]" lowering-algo))]))
