...
(define (fold-grammar lifted-sub-expr halide-expr [depth 0])
  (define candidates
    (destruct lifted-sub-expr
      ...
      [(vs-mpy-add sub-exprs weight-matrix output-type saturate?)
       ...
       (cond
         [(halide-cast-op? halide-expr)
          ...
          (list (vs-mpy-add (get-node-id) sub-exprs weight-matrix (halide-elem-type halide-expr) saturate?))]
         ...
         [(or (vec-mul? halide-expr) (vec-shl? halide-expr))
          ...
          (list (vs-mpy-add (get-node-id) sub-exprs updated-weights output-type saturate?))]
         [else
          (list (vs-mpy-add (get-node-id) sub-exprs weight-matrix output-type saturate?))])]
