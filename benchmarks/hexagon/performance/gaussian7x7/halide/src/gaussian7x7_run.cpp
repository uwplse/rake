...
int main(int argc, char **argv) {
  ...
  char *filename = (char *) malloc(100 * sizeof(char));
    ...
    halide_dimension_t x_dim{3, width-3, 1};
    halide_dimension_t y_dim{3, height-3, width};
    ...
    cycles_hvx64 = benchmark([&]() {
        int error = gaussian7x7_hvx64(input1_buf, output_buf);
        if (error != 0) {
          printf("gaussian7x7_hvx64 pipeline failed: %d\n", error);
        }
      });
    SIM_RELEASE_HVX;

    printf("AppReported (HVX64B-mode): Image %dx%d - gaussian7x7(64B): %lld cycles (%0.4f cycles/pixel)\n", (int)width, (int)height, cycles_hvx64, (float)cycles_hvx64/width/height);*/
    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    /*strcpy(filename, "out/gaussian7x7_hvx64_");
    ...
    strcpy(filename, "out/gaussian7x7_hvx128_");
    strcat(filename, argv[4]);
    ...
    cycles_basline_hvx64 = benchmark([&]() {
        int error = gaussian7x7u8_hvx64_i( input, stride, width, height, output);
        if (error != 0) {
          printf("gaussian7x7_baseline_hvx64 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX64B-mode): Image %dx%d - gaussian7x7_basline(64B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx64/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "gaussian7x7_baseline_hvx64_");
    strcat(filename, argv[4]);
    ...
    cycles_basline_hvx128 = benchmark([&]() {
        int error = gaussian7x7u8_hvx128_i( input, stride, width, height, output);
        if (error != 0) {
          printf("gaussian7x7_baseline_hvx128 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX128B-mode): Image %dx%d - gaussian7x7_basline(128B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx128/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "gaussian7x7_baseline_hvx128_");
    strcat(filename, argv[4]);
