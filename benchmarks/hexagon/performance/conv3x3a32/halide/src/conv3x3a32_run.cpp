...
int main(int argc, char **argv) {
  ...
  char *filename = (char *) malloc(100 * sizeof(char));
    ...
    halide_dimension_t xi_dim{0, width, 1};
    halide_dimension_t yi_dim{0, height, width};
    halide_dimension_t i_shape[2] = {xi_dim, yi_dim};

    halide_dimension_t xo_dim{1, width-1, 1};
    halide_dimension_t yo_dim{1, height-1, width};
    halide_dimension_t o_shape[2] = {xo_dim, yo_dim};
    ...
    Halide::Runtime::Buffer<uint8_t> input1_buf(input, dims, i_shape);
    Halide::Runtime::Buffer<int8_t> mask_buf(mask, dims, mask_shape);
    Halide::Runtime::Buffer<uint8_t> output_buf(output, dims, o_shape);

    SIM_ACQUIRE_HVX;
    cycles_hvx64 = benchmark([&]() {
        int error = conv3x3a32_hvx64(input1_buf, mask_buf, output_buf);
        if (error != 0) {
          printf("conv3x3a32_hvx64 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX64B-mode): Image %dx%d - conv3x3a32(64B): %lld cycles (%0.4f cycles/pixel)\n", (int)width, (int)height, cycles_hvx64, (float)cycles_hvx64/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "out/conv3x3a32_hvx64_");
    ...
    if(write_file(fp_hvx64, output, height, width, 1) != 0) {
    ...
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "out/conv3x3a32_hvx128_");
    strcat(filename, argv[4]);
    ...
    if(write_file(fp_hvx128, output, height, width, 1) != 0) {
      printf("Error: Cannot write to file %s\n", filename);
    }
    ...
    cycles_basline_hvx64 = benchmark([&]() {
        int error = conv3x3_hvx64_i( input, stride, width, height, mask_intrinsic, 4, output);
        if (error != 0) {
          printf("conv3x3a32_baseline_hvx64 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX64B-mode): Image %dx%d - conv3x3a32_basline(64B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx64/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "conv3x3a32_baseline_hvx64_");
    strcat(filename, argv[4]);
    ...
    if(write_file(fp_baseline_hvx64, output, height, width, 1) != 0) {
      printf("Error: Cannot write to file %s\n", filename);
    }
    ...
    cycles_basline_hvx128 = benchmark([&]() {
        int error = conv3x3_hvx128_i( input, stride, width, height, mask_intrinsic, 4, output);
        if (error != 0) {
          printf("conv3x3a32_baseline_hvx128 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX128B-mode): Image %dx%d - conv3x3a32_basline(128B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx128/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "conv3x3a32_baseline_hvx128_");
    strcat(filename, argv[4]);
    ...
    if(write_file(fp_baseline_hvx128, output, height, width, 1) != 0) {
      printf("Error: Cannot write to file %s\n", filename);
    }
