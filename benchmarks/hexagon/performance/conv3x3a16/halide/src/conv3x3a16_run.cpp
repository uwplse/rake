...
int main(int argc, char **argv) {
  ...
  char *filename = (char *) malloc(100 * sizeof(char));
    ...
    cycles_hvx64 = benchmark([&]() {
        int error = conv3x3a16_hvx64(input1_buf, mask_buf, output_buf);
        if (error != 0) {
          printf("conv3x3a16_hvx64 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX64B-mode): Image %dx%d - conv3x3a16(64B): %lld cycles (%0.4f cycles/pixel)\n", (int)width, (int)height, cycles_hvx64, (float)cycles_hvx64/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "out/conv3x3a16_hvx64_");
    ...
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "out/conv3x3a16_hvx128_");
    strcat(filename, argv[4]);
    ...
    cycles_basline_hvx64 = benchmark([&]() {
        int error = conv3x3_hvx64_i( input, stride, width, height, mask, 4, output, stride);
        if (error != 0) {
          printf("conv3x3a16_baseline_hvx64 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX64B-mode): Image %dx%d - conv3x3a16_basline(64B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx64/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "conv3x3a16_baseline_hvx64_");
    strcat(filename, argv[4]);
    ...
    cycles_basline_hvx128 = benchmark([&]() {
        int error = conv3x3_hvx128_i( input, stride, width, height, mask, 4, output, stride);
        if (error != 0) {
          printf("conv3x3a16_baseline_hvx128 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX128B-mode): Image %dx%d - conv3x3a16_basline(128B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx128/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "conv3x3a16_baseline_hvx128_");
    strcat(filename, argv[4]);
