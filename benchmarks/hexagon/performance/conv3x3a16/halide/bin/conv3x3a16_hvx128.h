#ifndef HALIDE__bin___conv3x3a16_hvx128_h
#define HALIDE__bin___conv3x3a16_hvx128_h
#include <stdint.h>
...
int conv3x3a16_hvx128(struct halide_buffer_t *_input_buffer, struct halide_buffer_t *_mask_buffer, struct halide_buffer_t *_output_buffer) HALIDE_FUNCTION_ATTRS;
int conv3x3a16_hvx128_argv(void **args) HALIDE_FUNCTION_ATTRS;
const struct halide_filter_metadata_t *conv3x3a16_hvx128_metadata() HALIDE_FUNCTION_ATTRS;
...
inline const struct halide_filter_metadata_t* (*HALIDE_GET_STANDARD_METADATA_FUNCTION())() {
    return conv3x3a16_hvx128_metadata;
}
#endif
...
typedef enum halide_target_feature_t {
    ...
    halide_target_feature_hvx_sysmon = 56, /// Enable sysmon markers
    halide_target_feature_hexagon_dma = 57, ///< Enable Hexagon DMA buffers.
...
typedef struct halide_buffer_t {
    ...
    uint64_t flags;                  // Using this to store ION allocation's fd when using use_host_ptr_extension is enabled
    uint64_t use_host_ptr_extension; // Using this to enable host_ptr extension
    uint64_t read_only;              // Using this to indicate to the runtime that this buffer should be read-only for the GPU
    uint64_t is_cached;              // Using this to indicate whether the cache policy of the underlying ION allocation
...
extern int halide_hexagon_set_malloc_tracing(void *user_context, int lev);

/** enable a null dereference for a list of errors passed in.
 * The list must be an array of ints terminated with a 0 value.
 * These should only be assigned halide_error_code_t enums as defined in HalideRuntime.h
 * if enable_all_errors is true, then lev_list is ignored and all errors are set to enabled
*/
extern int halide_hexagon_set_error_fault_mask(void *user_context, bool enable_all_errors, int *lev_list);
 ...
 *
 * min, minimum bytes to grow the heap by when requesting a new segment
 * max, maximum bytes to grow the heap by when requesting a new segment
 *
 * Internally, the value will be aligned to the next 1MB boundary.
 */

extern int halide_hexagon_mem_set_grow_size(void *user_context, long long int grow_min, long long int grow_max);
...
#else
extern int halide_hexagon_set_performance_mode(void *user_context, halide_hexagon_power_mode_t mode, bool dcvs_enable);
extern int halide_hexagon_set_performance(void *user_context, halide_hexagon_power_t *perf, bool dcvs_enable);
...
/** Initialize Unsigned PD mode for Hexagon.
  * This API needs to be called before running pipeline and before any other
  * hexagon related calls */
extern int halide_hexagon_init_unsigned_mode(void *user_context);

/** Allow program to allocate or cleanup ion FDs */
