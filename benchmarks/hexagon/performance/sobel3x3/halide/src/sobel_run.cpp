...
int main(int argc, char **argv) {
  ...
  char *filename = (char *) malloc(100 * sizeof(char));
    ...
    halide_dimension_t ioo_shape[2] = {xo_dim, yo_dim};

    Halide::Runtime::Buffer<uint8_t> input1_buf(input, dims, io_shape);
    Halide::Runtime::Buffer<uint8_t> output_buf(output, dims, ioo_shape);

    SIM_ACQUIRE_HVX;
    cycles_hvx64 = benchmark([&]() {
        int error = sobel_hvx64(input1_buf, output_buf);
        if (error != 0) {
          printf("sobel_hvx64 pipeline failed: %d\n", error);
        }
      });
    SIM_RELEASE_HVX;
    ...
    /* -----------------------------------------------------*/
    strcpy(filename, "out/sobel_hvx64_");
    ...
    cycles_hvx128 = benchmark([&]() {
        int error = sobel_hvx128(input1_buf, output_buf);
        if (error != 0) {
          printf("sobel_hvx128 pipeline failed: %d\n", error);
        }
      });
    SIM_RELEASE_HVX;
    ...
    cycles_basline_hvx64 = benchmark([&]() {
        int error = sobel_hvx64_i( input, stride, width, height, output);
        if (error != 0) {
          printf("sobel_baseline_hvx64 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX64B-mode): Image %dx%d - sobel_basline(64B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx64/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "sobel_baseline_hvx64_");
    strcat(filename, argv[4]);
    ...
    cycles_basline_hvx128 = benchmark([&]() {
        int error = sobel_hvx128_i( input, stride, width, height, output);
        if (error != 0) {
          printf("sobel_baseline_hvx128 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX128B-mode): Image %dx%d - sobel_basline(128B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx128/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "sobel_baseline_hvx128_");
    strcat(filename, argv[4]);
