...
(define t8 (+ (* rows.s0.x.x 128) (* (+ (* output.s0.y.y 4) 1) input.stride.1)))
...
(define halide-expr
 (vec-add
  (vec-add
   (vec-mul
    (int16x128
     (load input (ramp (+ t8 -1) 1 128) (aligned 128 127)))
    (x128 (int16_t (bv 2 16))))
   (int16x128
    (load input (ramp (+ (+ (* input.stride.1 -1) t8) -1) 1 128) (aligned 1 0))))
  (int16x128
   (load input (ramp (+ (+ input.stride.1 t8) -1) 1 128) (aligned 128 127)))))
