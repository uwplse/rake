...
(define t20 (+ (* rows.s0.x.x 128) (* (+ (* output.s0.y.y 4) 1) input.stride.1)))
 ...
 (vec-add
  (vec-add
   (vec-add
    (vec-mul
     (int16x128
      (load input (ramp (+ (+ (* input.stride.1 -1) t20) -2) 1 128) (aligned 1 0)))
     (x128 (int16_t (bv 4 16))))
    (vec-add
     (vec-mul
      (int16x128
       (load input (ramp (+ t20 -2) 1 128) (aligned 128 126)))
      (int16x128
       (x128 (int8_t (bv 6 8)))))
     (vec-mul
      (int16x128
       (load input (ramp (+ (+ input.stride.1 t20) -2) 1 128) (aligned 128 126)))
      (x128 (int16_t (bv 4 16))))))
   (int16x128
    (load input (ramp (+ (+ (* input.stride.1 -2) t20) -2) 1 128) (aligned 128 126))))
  (int16x128
   (load input (ramp (+ (+ (* input.stride.1 2) t20) -2) 1 128) (aligned 128 126)))))
