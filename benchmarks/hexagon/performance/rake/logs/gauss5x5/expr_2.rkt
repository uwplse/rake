...
(define t21 (+ (* rows.s0.x.x 128) (- (* (+ (* output.s0.y.y 4) 2) input.stride.1) t33)))
 ...
 (vec-add
  (vec-add
   (vec-add
    (vec-mul
     (int16x128
      (load input (ramp (+ (+ (* input.stride.1 -1) t21) -2) 1 128)))
     (x128 (int16_t (bv 4 16))))
    (vec-add
     (vec-mul
      (int16x128
       (load input (ramp (+ (+ input.stride.1 t21) -2) 1 128)))
      (x128 (int16_t (bv 4 16))))
     (vec-mul
      (int16x128
       (load input (ramp (+ t21 -2) 1 128)))
      (x128 (int16_t (bv 6 16))))))
   (int16x128
    (load input (ramp (+ (+ (* input.stride.1 -2) t21) -2) 1 128))))
  (int16x128
   (load input (ramp (+ (+ (* input.stride.1 2) t21) -2) 1 128)))))
