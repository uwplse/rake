...
(define t19.s (+ (* rows.s0.x.x 32) (* input.stride.1 output.s0.y.y)))
 ...
 (vec-add
  (vec-add
   (vec-add
    (vec-mul
     (int16x128
      (load input (ramp (+ (+ (* input.stride.1 -1) (- (* t19.s 4) t33)) -2) 1 128)))
     (x128 (int16_t (bv 4 16))))
    (vec-add
     (vec-mul
      (int16x128
       (load input (ramp (+ (+ (- (* t19.s 4) t33) input.stride.1) -2) 1 128)))
      (x128 (int16_t (bv 4 16))))
     (vec-mul
      (int16x128
       (load input (ramp (+ (- (* t19.s 4) t33) -2) 1 128)))
      (x128 (int16_t (bv 6 16))))))
   (int16x128
    (load input (ramp (+ (+ (* input.stride.1 -2) (- (* t19.s 4) t33)) -2) 1 128))))
  (int16x128
   (load input (ramp (+ (+ (* input.stride.1 2) (- (* t19.s 4) t33)) -2) 1 128)))))
