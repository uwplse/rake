...
(define t6.s (+ (* output.s0.x.x 32) (* input.stride.1 output.s0.y.y)))
(define t18 (+ (* input.stride.1 -1) t30))
         ...
         (vec-add
          (vec-add
           (vec-add
            (vec-mul
             (int16x128
              (load input (ramp t30 1 128)))
             (x128 (int16_t (bv 4 16))))
            (vec-mul
             (int16x128
              (load input (ramp t18 1 128)))
             (x128 (int16_t (bv 2 16)))))
           (vec-mul
            (int16x128
             (load input (ramp (+ t30 input.stride.1) 1 128)))
            (x128 (int16_t (bv 2 16)))))
          (vec-mul
           (int16x128
            (load input (ramp (+ t30 -1) 1 128)))
           (x128 (int16_t (bv 2 16)))))
         (int16x128
          (load input (ramp (+ t18 -1) 1 128))))
        (int16x128
         (load input (ramp (+ (+ t30 input.stride.1) -1) 1 128)))))
      (int16x128
       (load input (ramp (+ t18 1) 1 128))))
     (int16x128
      (load input (ramp (+ (+ t30 input.stride.1) 1) 1 128))))
