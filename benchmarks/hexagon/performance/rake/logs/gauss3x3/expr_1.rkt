...
(define t9 (+ (* output.s0.x.x 128) (- (* (+ (* output.s0.y.y 4) 1) input.stride.1) t30)))
         ...
         (vec-add
          (vec-add
           (vec-add
            (vec-mul
             (int16x128
              (load input (ramp t9 1 128)))
             (x128 (int16_t (bv 4 16))))
            (vec-mul
             (int16x128
              (load input (ramp (+ (* input.stride.1 -1) t9) 1 128)))
             (x128 (int16_t (bv 2 16)))))
           (vec-mul
            (int16x128
             (load input (ramp (+ input.stride.1 t9) 1 128)))
            (x128 (int16_t (bv 2 16)))))
          (vec-mul
           (int16x128
            (load input (ramp (+ t9 -1) 1 128)))
           (x128 (int16_t (bv 2 16)))))
         (int16x128
          (load input (ramp (+ (+ (* input.stride.1 -1) t9) -1) 1 128))))
        (int16x128
         (load input (ramp (+ (+ input.stride.1 t9) -1) 1 128)))))
      (int16x128
       (load input (ramp (+ (+ (* input.stride.1 -1) t9) 1) 1 128))))
     (int16x128
      (load input (ramp (+ (+ input.stride.1 t9) 1) 1 128))))
